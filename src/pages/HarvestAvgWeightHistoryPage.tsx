...
export default function HarvestAvgWeightHistoryPage() {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      try {
        ...
        const avgWeightByDate = trucksUntilSelectedDate.reduce<Record<string, { birds: number; netWeight: number }>>((acc, truck) => {
        ...
        for (let cursor = firstTruckDate; cursor.isBefore(endDate) || cursor.isSame(endDate, "day"); cursor = cursor.add(1, "day")) {
          ...
          nextRows.push({
            ...
            avgWeight,
            birdsLoaded: avgWeightData?.birds ?? 0,
          });
        }
    ...
    <Layout className="min-h-screen bg-slate-100">
      ...
      <Content className={isMobile ? "px-3 py-3 pb-8" : "px-8 py-6"}>
        {isLoading ? (
          ...
          <ChickenState
            title="No avg weight yet"
            subtitle={`No harvest weight data found from day 1 up to ${dayjs(selectedDate).format("MMMM D, YYYY")}.`}
            fullScreen
          />
        ) : (
          <>
